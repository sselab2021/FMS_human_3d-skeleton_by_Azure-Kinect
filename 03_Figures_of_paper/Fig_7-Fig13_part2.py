"""
    # Fig_X_x (movementID, [a, b, c])
    # this .py script is suitable for movements and joints as follows:
        Fig_7_2 (m01, [18, 19, 20]), Fig_7_4 (m02, [18, 19, 20])
        Fig_8_2 (m03, [18, 19, 20])
        Fig_9_2 (m05, [18, 19, 20])
        Fig_13_2 (m12, [18, 19, 20])
        Fig_13_4 (m14, [22, 23, 24])
"""
...
for index, per_json in enumerate(valid_episodes_json_files_list):
    ...
    for i in range(json_dict["total_frames"]):
        ...
        # ∠abc
        joint_1_xyz = np.array(json_dict["frames"][i]["bodies"][0]["joint_positions"][19])  # a
        joint_2_xyz = np.array(json_dict["frames"][i]["bodies"][0]["joint_positions"][18])  # b
        joint_3_xyz = np.array(json_dict["frames"][i]["bodies"][0]["joint_positions"][20])  # c

        # the vectors of above three joints.
        vector1 = joint_2_xyz - joint_1_xyz
        vector2 = joint_3_xyz - joint_1_xyz
